...
BEGIN
  ...
  END IF;
  -- Embalagem (custo gerencial — NÃO gera SAÍDA financeira)
  -- A regra de negócio definitiva diz:
  --   dinheiro já saiu quando materiais foram COMPRADOS.
  --   Aqui o valor reduz apenas real_profit via sales.packaging_cost.
  -- Portanto NÃO inserimos em financial_transactions.
  -- O valor continua gravado em sales.packaging_cost e compõe real_profit normalmente.
  -- Custos extras (saída)
...
BEGIN
  ...
  -- funding_source OFICIAL: 4 valores apenas (item #6 fechamento)
  ...
  --   CAIXA_EVELINE → saídas financeiras com payment_source='CAIXA_EVELINE'
  --   FABIANA/DONA/OUTRO → obligation PENDENTE atrelada (sem saída no caixa)
  -- ============================================================
